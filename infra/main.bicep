...
@description('Location for the AI Services account and model deployment. Must be a region that offers the model on the GlobalStandard SKU (this list is pre-filtered to those regions). Pick a region close to your Function region.')
@metadata({
  azd: {
    type: 'location'
  }
...
output AZURE_FUNCTION_MI_PRINCIPAL_ID string = apiUserAssignedIdentity.outputs.principalId
output AZURE_FUNCTION_MI_CLIENT_ID string = apiUserAssignedIdentity.outputs.clientId
