...
- **For deployment**, an azd `prepackage` hook (in [`azure.yaml`](azure.yaml)) runs `npm install --os=linux --cpu=x64`, which downloads the **Linux** CLI binary into `node_modules/` (even when you deploy from macOS/Windows). That folder is included in the deployment package, so the binary is present at runtime. **This requires Node.js + npm on the machine you run `azd` from.**
